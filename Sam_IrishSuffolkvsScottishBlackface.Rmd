...
```{r}
extract.breeds <- function(bfileBasename, clusters) {
  system(paste("plink --bfile", bfileBasename,
               "--chr-set 26 --autosome",
               "--fst --within clusterfst.txt",
               "--out DatasetFst",
               sep = " "))
}

data <- extract.breeds("Dataset", "clusterfst.txt")
```
...
```{r data}
std.fst <- function(data, outfile) {
  errorCondition("not implemented")
}

data.fst <- std.fst(data, "Sam_IrishSuffolkvsScottishBlackface_FST.png")
```
